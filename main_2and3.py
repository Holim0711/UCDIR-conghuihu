...
import loader
import builder_2and3 as builder
import json
...
parser.add_argument('--domain-weight', type=float, default=1.0, help='domain loss weight for method3')
#################
...
def main_worker(gpu, args):
    ...
    if args.method == 'method3' or args.method == 'method2and3':
        criterion_domain = nn.NLLLoss().cuda(args.gpu)
        ...
        if os.path.isfile(args.clean_model):
            ...
            if args.method == 'method2':
                for k in current_state:
                    ...
                    if 'final_fc' in k: continue ### fc 모양이 다름 2048 != 1536
                ...
                for k in current_state:
                    if 'queue' in k: continue
                    if 'domain_classifier' in k: continue
                    if 'featurizer' in k:
                        k_replace = k.replace("featurizer.","")
                        k_parts = '.'.join(k_replace.split('.')[1:])
                        used_pretrained_state[k] = clean_checkpoint['state_dict']['module.encoder_q.'+k_parts]
                    if 'final_fc' in k:
                        k_replace = k.replace("final_fc","fc")
                        k_parts = '.'.join(k_replace.split('.')[1:])
                        used_pretrained_state[k] = clean_checkpoint['state_dict']['module.encoder_q.'+k_parts]
            elif args.method == 'method2and3':
                for k in current_state:
                    if 'queue' in k: continue
                    if 'domain_classifier' in k: continue
                    if 'avgpool' in k or 'gempool' in k or 'maxpool' in k: continue
    ...
    for epoch in range(args.epochs):
        ...
        ### p = (0, 1) 점점 0에서 1로 linear하게 가는 수
        ### alpha도 점점 0에서 1로 가는 수 (non-linear)
        ...
        if args.method == 'method3' or args.method == 'method2and3':
            train(train_loader, model, criterion, optimizer, epoch, args, info_save, cluster_result, alpha, criterion_domain)
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, args, info_save, cluster_result, alpha=None, criterion_domain=None):

    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')

    if not criterion_domain:
        ...
        losses = {'Inst_A': AverageMeter('Inst_Loss_A', ':.4e'),
                  'Inst_B': AverageMeter('Inst_Loss_B', ':.4e'),
                  'Domain_A' : AverageMeter('Domain_loss_A', ':.4e'),
                  'Domain_B' : AverageMeter('Domain_loss_B', ':.4e'),
                  'Cwcon_A': AverageMeter('Cwcon_Loss_A', ':.4e'),
                  'Cwcon_B': AverageMeter('Cwcon_Loss_B', ':.4e'),
                  'SelfEntropy': AverageMeter('Loss_SelfEntropy', ':.4e'),
                  'DistLogits': AverageMeter('Loss_DistLogits', ':.4e'),
    ...
    for i, (images_A, image_ids_A, images_B, image_ids_B, cates_A, cates_B) in enumerate(train_loader):
        ...
        if args.method == 'method3' or args.method == 'method2and3':
            ...
            losses_distlogits, \
            losses_cwcon, \
            losses_domain = model(im_q_A=images_A[0], im_k_A=images_A[1],
                                 im_id_A=image_ids_A, im_q_B=images_B[0],
                                im_k_B=images_B[1], im_id_B=image_ids_B,
                                cluster_result=cluster_result,
                                criterion=criterion, alpha=alpha, criterion_domain=criterion_domain)
        else:
            losses_instcon, \
            q_A,  q_B, \
            losses_selfentro, \
        ...
        if args.method == 'method3' or args.method == 'method2and3':
            d_loss_A = losses_domain['domain_loss_A']
            d_loss_B = losses_domain['domain_loss_B']
            losses['Domain_A'].update(d_loss_A.item(), images_A[0].size(0))
            losses['Domain_B'].update(d_loss_B.item(), images_B[0].size(0))
            d_l_A = d_loss_A * args.domain_weight
            d_l_B = d_loss_B * args.domain_weight
            all_loss += (d_l_A + d_l_B) / 2
...
def compute_features(eval_loader, model, args, withoutfc=False):
    ...
    dim = args.low_dim
    if args.method == 'method2and3' : alpha = args.alpha
    if withoutfc:
        ...
        if args.method == 'default':
            ...
            featurizer = builder.method3_encoder(models.__dict__[args.arch]).featurizer
            state_dict = model.encoder_k.featurizer.state_dict()
            state_dict = {k: state_dict[k] for k in featurizer.state_dict()}
            featurizer.load_state_dict(state_dict)
            featurizer.eval()
            featurizer.cuda()
            ...
            featurizer = builder.method2and3encoder(args.smg, models.__dict__[args.arch])
            state_dict = model.encoder_k.state_dict()
            state_dict = {k: state_dict[k] for k in featurizer.state_dict()}
            featurizer.load_state_dict(state_dict)
            featurizer.eval()
    ...
    if args.method == 'method2and3' : dim = 1536
        ...
        with torch.no_grad():
            ...
            if withoutfc:
                if args.method == 'default':
                    ...
                    feats_A, _ = featurizer(images_A, alpha)
                    feats_B, _ = featurizer(images_B, alpha)
            else:
                if args.method == 'method2and3':
                    feats_A, feats_B = model(im_q_A=images_A, im_q_B=images_B, is_eval=True, alpha=alpha)
                else:

...
class method2and3encoder(nn.Module):
    def __init__(self, smg, base_encoder, dim=128, total_dim=1536, norm_layer=None):
        super(method2and3encoder, self).__init__()
        ...
        ### remove downsampling
        self.featurizer.layer3[0].conv2 = nn.Conv2d(256,256,kernel_size=3,stride=1,padding=1,groups=1,bias=False,dilation=1)
        self.featurizer.layer3[0].downsample[0] = nn.Conv2d(512,1024,kernel_size=1,stride=1,bias=False)

        self.featurizer.avgpool = nn.Identity()
        self.featurizer.fc = nn.Unflatten(1, (2048, 14, 14))
...
class UCDIR(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K_A=65536, K_B=65536,
        ...
        if self.method == 'default':
            ...
            for param_q, param_k in zip(self.encoder_q.parameters(), self.encoder_k.parameters()):
                param_k.data.copy_(param_q.data)  # initialize
                param_k.requires_grad = False  # not update by gradient
        elif self.method == 'method2and3':
            self.smg = smg
            if self.smg is None:
                print('method 2and3 needs args.smg')
                exit()

            self.encoder_q = method2and3encoder(self.smg, base_encoder, dim=128, total_dim=1536)
            self.encoder_k = method2and3encoder(self.smg, base_encoder, dim=128, total_dim=1536, norm_layer=norm_layer)
    ...
    def forward(self, im_q_A, im_q_B, im_k_A=None, im_id_A=None,
        ...
        if self.method == 'default':
            ...
            im_q = torch.cat([im_q_A, im_q_B], dim=0)
            if is_eval:
                k, _ = self.encoder_k(im_q, alpha)
                #k = F.normalize(k, dim=1)
                k_A, k_B = torch.split(k, im_q_A.shape[0])
                return k_A, k_B
            if alpha is None:
                print("in method2and3, train mode need alpha value")
                exit()
            if criterion_domain is None:
                print("in method2and3, train mode need criterion_domain")
                exit()
            q, d = self.encoder_q(im_q, alpha)
            #q = F.normalize(q, dim=1)
            q_A, q_B = torch.split(q, im_q_A.shape[0])
        ...
        if cluster_result is not None:
            ...
            if self.method == 'method3' or self.method == 'method2and3':

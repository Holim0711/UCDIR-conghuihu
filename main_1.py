...
import loader_new as loader
import builder_1 as builder
...
parser.add_argument('--dataset', metavar='Dataset name', help='Name of dataset. ex) DomainNet, OfficeHome')
parser.add_argument('--domainA', metavar='Domain A', help='Domain A. ex) clipart')
parser.add_argument('--domainB', metavar='Domain B', help='Domain B. ex) Art')
parser.add_argument('--is_split', type=str2bool, default=False, metavar='is_split', help='used split data.')
...
def main():
    ...
    if not os.path.exists(f'./results/method1/{args.gpu}/{args.exp_dir}'):
        os.makedirs(f'./results/method1/{args.gpu}/{args.exp_dir}', exist_ok=True)
...
def main_worker(gpu, args):
    ...
    print(f"is_split : {args.is_split}, withoutfc : {args.withoutfc}")
    ...
    train_dataset = loader.TrainDataset(args.dataset, args.domainA, args.domainB, args.aug_plus, is_split)
    train_dataset_d = loader.TrainDetDataset(args.dataset, args.domainA, args.domainB, is_split)
    eval_dataset = loader.EvalDataset(args.dataset, args.domainA, args.domainB, is_split)
    ...
    info_save = open(os.path.join(f'./results/method1/{args.gpu}/{args.exp_dir}', 'info.txt'), 'w')
    score_save = open(os.path.join(f'./results/method1/{args.gpu}/{args.exp_dir}', 'score.txt'), 'w')
    best_res_A = [0., 0., 0., 0.]
    best_res_B = [0., 0., 0., 0.]
    best_ndcg_A = [0., 0., 0., 0.]
    best_ndcg_B = [0., 0., 0., 0.]
    ...
    for epoch in range(args.epochs):
        ...
        #if total_loss > train_total_loss:
        #    total_loss = train_total_loss
        #    best_state = model.state_dict()

        res_A, res_B, ndcg_A, ndcg_B, meanap_A, meanap_B = test(eval_loader, model, args, withoutfc=args.withoutfc)
    ...
    save_checkpoint(model.state_dict(), False, f'./results/method1/{args.gpu}/{args.exp_dir}/checkpoint.pth.tar')

def test(eval_loader, model, args, withoutfc=False):

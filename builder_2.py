...
class method2_2encoder(nn.Module):
    def __init__(self, smg, base_encoder, dim=128, total_dim=1536, norm_layer=None):
        super(method2_2encoder, self).__init__()
...
class UCDIR(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K_A=65536, K_B=65536,
        ...
        elif self.method == 'method2_2':
            self.smg = smg
            if self.smg is None:
                print('method 2 needs args.smg')
                exit()

            self.encoder_q = method2_2encoder(self.smg, base_encoder, dim=128, total_dim=1536)
            self.encoder_k = method2_2encoder(self.smg, base_encoder, dim=128, total_dim=1536, norm_layer=norm_layer)
